...
import com.chrrissoft.marvel.ui.series.SeriesScreenState.PreviewResState.*
...
class SeriesViewModel @Inject constructor(
    ...
    private fun updatePrevRes(res: SeriesPrevRes) {
        _uiState.update {
            when (res.state) {
                is SeriesPrevResState.Error -> {
                    it.copy(
                        previews = res.state.data,
                        previewResState = ERROR
                    )
                }
                is SeriesPrevResState.Loading -> {
                    it.copy(
                        previews = res.state.data,
                        previewResState = LOADING
                    )
                }
                is SeriesPrevResState.Success -> {
                    it.copy(
                        previews = res.state.data,
